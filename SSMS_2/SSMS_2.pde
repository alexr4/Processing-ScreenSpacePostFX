...
public void setup() {
  ...
  threshold = loadShader(dataPath+"threshold.glsl");
...
public void draw() {
  ...
  float lfar = 750;
  lightDir.set(sin(lightAngle) * 0.5 + 0.5, 0, cos(lightAngle) * 0.5 + 0.5);
  ...
  threshold.set("level", 0.85);
  threshold.set("depthmap", depth);
  passssms.getCustomFilter(passfog.getBuffer(), threshold); 
  ...
  //comp.getBlendAdd(passssms.getBuffer(),passfog.getBuffer(), 100.0);
  //comp.getBlendAverage(passssms.getBuffer(),passfog.getBuffer(), 100.0);
  comp.getBlendOverlay(passssms.getBuffer(), passfog.getBuffer(), 100.0);
  //comp.getBlendSoftLight(passssms.getBuffer(),passfog.getBuffer(), 100.0);
  image(comp.getBuffer(), 0, 0);
  //image(albedo, 0, 0);
