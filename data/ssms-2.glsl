...
void main(){
    ...
    float noised = (0.25 + fbm(vec3(uv, fogFactor) + time, 0.75, 5.5, 0.25, 0.75) * 0.75) * fogFactor;
    noised = clamp(noised, 0.0, 1.0);

    finalColor =  getBlur9x9(uv, texture, fogFactor * 10.0 + noised * 10.0, blurDir.x >= 1.0 ? 1.0 / resolution.x : 1.0 / resolution.y , blurDir);
