...
vec3 depthOfField(vec2 texCoord, float focusPoint, float focusScale)
{
    ...
    vec2 rnd = texture2D(bluenoise, fract(texCoord * 4.0)).xy * 0.5;
	float centerDepth = ldepth(texture2D(depthmap, texCoord).r) * far;
	float centerSize = getBlurSize(centerDepth, focusPoint * rnd.x, focusScale);
    float asum = 0.0;
    vec3 col = vec3(0.0);
    for(float t = 0.0; t < 8.0 * 2.0 * 3.14; t += 3.14 / 32.0) {
	    ...
	    float sampSize = getBlurSize(sampDepth, focusPoint * rnd.x, focusScale);
	    ...
	    sampSize = getBlurSize(sampDepth, focusPoint * rnd.x, focusScale);
