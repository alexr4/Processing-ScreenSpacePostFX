...
vec3 getNormal(float depth, vec2 uv, sampler2D depthmap){
	vec2 offsety = vec2(0.0, 0.00001);
	vec2 offsetx = vec2(0.00001, 0.0);

	float depthoy = texture2D(depthmap, fract(uv + offsety)).r;
	float depthox = texture2D(depthmap, fract(uv + offsetx)).r;
...
float getSSAO(vec2 uv, sampler2D depthmap){
    ...
    vec3 normal = reconstructPosition(depth, uv, InvVP);
